    ...
    try:
        ...
        if isinstance(data, list) and all(isinstance(item, dict) for item in data):
            ...
            for item in data:
                ...
                # ✅ Price calculation
                try:
                    unit_price = float(match.get("Price", 0))
...
# ====================================================
    ...
    if any(word in user_msg_lower for word in menu_keywords):
        ...
        # Limit to top 10 dishes for cleaner display
        menu_preview = "\n".join(
            [f"• {m['Dish']} — ₹{m['Price']}" for m in menu_data[:20]]
        )
        return {"response": f"📋 Here's a look at our menu:\n\n{menu_preview}\n\nWould you like to order something?"}
...
async def get_menu():
    try:
        ...
        result = sheet.values().get(
            ...
            range="menu!A2:D999"  # Change 'menu' to your sheet name
        ).execute()
...
async def chatbot(req: ChatRequest):
    ...
    try:
        ...
        intent_raw = intent_response.text.strip().lower()
        ...
        if intent not in valid_intents:
            ...
            """
            try:
                recheck_response = model.generate_content(recheck_prompt)
                intent = recheck_response.text.strip().lower()
                if intent not in valid_intents:
                    intent = "general_chat"
            except Exception as e:
    ...
    if intent == "book_table":
        ...
        if menu_data:  # if menu is loaded
            menu_preview = "\n".join(
                [f"• {m['Dish']} — ₹{m['Price']}" for m in menu_data[:15]]
            )
            return {
                "response": (
                    "🍽️ We serve **purely Indian vegetarian cuisine**, focusing on rich gravies and classic dishes like "
                    "Dal Tadka, Paneer Butter Masala, and more! 🥘\n\n"
                    "Here’s our menu:\n"
                    f"{menu_preview}\n\n"
